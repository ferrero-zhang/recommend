...
public class ExtractLog {
	...
	public static HashMap<String, String> ExtractAppLog(String fileName) {
		...
		while (rawUserData != null) {
			...
			try {
				...
				platform = GetPlatform(rawUserDataItems[2].trim());
				ip=rawUserDataItems[1].trim();
				net=GetNet(rawUserDataItems[7].trim());

				// 组合各字段值
				value = ip+"\t"+platform + "\t" + net+"\t"+action;
	 ...
	 * @return
	 */
	public static String GetPlatform(String mos) {
		String platform = mos.toLowerCase();
		if (platform.contains("android"))
			...
			platform = "ipa";
		else if (platform.contains("windowsphone"))
			platform = "w";
		else
			platform = "o";
		return platform;
