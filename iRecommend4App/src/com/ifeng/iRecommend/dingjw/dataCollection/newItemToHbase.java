...
import com.ifeng.iRecommend.fieldDicts.*;
	 ...
	 * -----------------------------------------------------------------------------
	 *          1.0          2015-1-12       hexl1          create
	 * -----------------------------------------------------------------------------
	 * </PRE>
	 */
public class newItemToHbase {

	private static newItemToHbase instance = new newItemToHbase();
		...
		public static newItemToHbase getInstance(){
			return instance;
		}
		...
		public static Item getItem(String id)
		{
			...
			try
			{
				item = itemOP.getItem(id);
			}
			catch(Exception e)
			{
				LOG.error("[ERROR] Get item "+id+" error.");
			}
			return item;
		}
		...
		public List<String> pretreatmentOfXml(StringBuffer sb)
		{
			...
			//	System.out.println(textSrc);//sb.toString().substring(0, 300)
				if(textSrc==null)
					textSrc = "";
			...
			//用splidWordsInterface进行分词
			//String textSplited = new String(splidWordsInterface.getInstance().segment(splitBuffer.toString().replace("•", "")).replace("(/", "_").replace(")", ""));
			//String textSplits[] = textSplited.split("\\|\\|_nx");
			return textList;
		}
	 ...
	 */
		...
		public int[] processXML(String dirpath, boolean delete){
			...
			LOG.info("[INFO]process id is "+id);
			...
			try{
				...
				try{
					if(item == null)
					{
						LOG.info("[INFO] Cann't get item "+id+" from IKV, extract it from xml.");
						hm_ItemInfo = ParseXmlForInfo(pretreatmentOfXml(xmlOpen(dirpath)),xmlOpen(dirpath));
					}
					else
					{
						hm_ItemInfo = item.toHashMap();
						LOG.info("[INFO] Get item "+id+" from IKV.");
					}						
					String url=null;
...
//						commenFuncs.deleteDirectory(dirpath);
			...
			while(true)
			{
				for(int i=0;i<typePaths.length;i++)
				{
					try{
						...
						LOG.info("[INFO] Now dirpath is "+dirpath);
		...
		public static void main(String[] args) {
			...
			newItemToHbase cItemToHbase = new newItemToHbase();
