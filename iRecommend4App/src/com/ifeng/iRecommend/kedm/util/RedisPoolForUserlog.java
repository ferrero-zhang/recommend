 ...
 * </PRE>
 */
public class RedisPoolForUserlog {
	private static final Log log = LogFactory.getLog(RedisPoolForUserlog.class);
    ...
    private static JedisPool jedisPool_app = null;//用于新版app用户模型redis，目前存放增量userid到redis
	...
	private static void redisPoolInit() {
		...
		String host_app = REDIS_CONFIG.getString("redis_host_app", "localhost");
		System.out.println(host_app);
		...
		try {
			...
			jedisPool_app = new JedisPool(config, host_app, port, timeout);
		} catch (Exception ex) {
    ...
    /**
     * @description 获取专为新用户体系app的jedis客户端
     * @param @return
     * @return Jedis
     */
    public synchronized static Jedis getJedisClientApp() {
        if (jedisPool_app == null) {
            redisPoolInit();
        }
        ...
        try {
            return jedisPool_app.getResource();
        } catch (Exception e) {
            log.error("Get jedis instance failed", e);
            return null;
        }
    ...
    public static void returnResourceApp(final Jedis jedis) {
        if (jedis != null && jedisPool_app != null) {
            jedisPool_app.returnResource(jedis);
        }
    }
	 ...
	 */
	public static Long sadd(String key,String value,int dbID,int outtime) {
		Long rt = -1L;
	...
	/**
	 * @description 批量将增量的useridSet发送到redis中，注意营养jedisPool_app
	 * @param @param uids
	 * @param @param dbID
	 * @return void
	 */
	public static void addBatch(Set<String> uids,String value,int dbID,int outtime){
		if(dbID < 0)
			return ;
		...
		boolean borrowOrOprSuccess = true;
		try {
			jedis = getJedisClientApp();
			jedis.select(dbID);
			Pipeline pipeline = jedis.pipelined();
			for(String key : uids){
				//pipeline.sadd("userIdsSet", value);
				pipeline.set(key, value);
				//pipeline.expire(key, outtime);
			}
			...
			returnResourceApp(jedis);
		}
	}
    public static void main(String[] args) {
    	Jedis jedis = getJedisClientApp();
    	jedis.select(13);
    	jedis.flushDB();
    	/*int dbNum = 8;
    	Gson gson = new Gson();
