...
public class DataConsistencyChecker extends TimerTask{
	...
	public void init() {
		try {
			// 默认超时时间正常情况够了，不用设置
			databaseJedis = new Jedis(CommonPSParams.commonDataRedisHost, CommonPSParams.commonDataRedisPort);
		} catch (Exception e) {
...
/*		List<String> blacklist_in_app = new CopyOnWriteArrayList<>(); 
		blacklist_in_app = BlackListData.getInstance().get_blacklist(); 
		...
		if (redis_black_list.size() != blacklist_in_app.size()) {
			...
			if (blacklist_in_app.size() < redis_black_list.size()) {
				...
				logger.error("Has Loss Blacklist Elems:" + redis_black_list.toString());
				System.out.println("Has Loss Blacklist Elems:" + redis_black_list.toString());
			} else {
				blacklist_in_app.removeAll(redis_black_list);
				logger.error("App Blacklist Has More Elems:" + blacklist_in_app.toString());
				System.out.println("App Blacklist Has More Elems:" + blacklist_in_app.toString());
			}
		}*/
	...
	public static void main(String[] args) {
		...
		timer.schedule(new DataConsistencyChecker("rec4user"), delay1, period1);
	}
}
